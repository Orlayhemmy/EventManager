...
export function setActivity(data) {
  return (dispatch) => {
    ...
    return axios.post('api/v1/activity', data).then((response) => {
      dispatch({ type: actionTypes.SET_ACTIVITY_SUCCESS, payload: response });
      dispatch(setAdminActivity(data));
    }).catch((err) => {
      dispatch({ type: actionTypes.SET_ACTIVITY_FAILS, payload: err.response.data });
    });
  };
}
...
export function deleteActivity(id) {
  return (dispatch) => {
    ...
    return axios.delete(`api/v1/activity/${id}`).then((response) => {
      dispatch({ type: actionTypes.DELETE_ACTIVITY_SUCCESS, payload: response });
    }).catch((err) => {
      dispatch({ type: actionTypes.DELETE_ACTIVITY_FAILS, payload: err.response.data });
    });
  };
}
